function plot_hen_grid_DRL(results, streams)
% Plot Heat Exchanger Network Grid Diagram - Clean Production Version
% Optimized: Fullscreen, no white space, adjusted label spacing, optimized legend layout
% Removed HU/CU text labels for clean visualization
%
...
% --- 0. Local temperature profile calculation ---
...
y_spacing = 3.5;           % Vertical spacing between streams
x_spacing = 100;           % Horizontal spacing between units
font_name = 'Times New Roman';
font_size_T = 9;           % Temperature text size
font_size_ID = 13;         % Stream ID text size
font_size_F = 11;          % Flowrate text size
font_size_Area = 9;        % Area text size
marker_size = 8;           % Marker size for connection points
extension_len = 60;        % Stream line extension length

% Text offset parameters
offset_area_y = 1.2;       % Vertical offset for area text
offset_temp_y = 0.5;       % Vertical offset for temperature text
offset_temp_x = 10;        % Horizontal offset for temperature text

% --- 2. Coordinate mapping logic ---
all_x_logical = [];
...
for i = 1:I+J
    ...
    % Plot inlet and outlet temperatures
...
if ~isempty(sm)
    for k = 1:height(sm)
        ...
        % Plot heat transfer area
        text(x, y2 + offset_area_y, sprintf('A=%.1f', hex_info.Area), 'HorizontalAlignment', 'center', 'FontName', font_name, 'FontSize', font_size_Area, 'Color', 'k', 'FontWeight', 'bold');
    end
end

% --- 7. Draw utilities (HU/CU text labels removed) ---
if ~isempty(results.cold_utilities)
    for k=1:height(results.cold_utilities)
        ...
        if ~isempty(node_match) && isKey(x_map_plot, node_match(1).x_pos)
            ...
            % Plot utility area (CU label removed)
            text(x, y + offset_area_y, sprintf('A=%.1f', util.Area), 'Color', 'b', 'FontName', font_name, 'FontSize', font_size_Area, 'HorizontalAlignment', 'center');
        end
    end
end
...
if ~isempty(results.hot_utilities)
    if isKey(x_map_plot, 0)
        ...
        for k=1:height(results.hot_utilities)
            ...
            % Plot utility area (HU label removed)
            text(x, y + offset_area_y, sprintf('A=%.1f', util.Area), 'Color', 'r', 'FontName', font_name, 'FontSize', font_size_Area, 'HorizontalAlignment', 'center');
        end
    end
end
...
%% --- Helper Function: Local Temperature Profile Calculation ---
function results = calculate_profiles_locally(results, streams)
    ...
    % Assign x positions for process heat exchangers
    ...
    for i = 1:height(sm_sorted)
        ...
        % Record heat exchange data for cold stream
    ...
    % Process cold streams in reverse order
    for j = 1:J
    ...
    % Extend stream profiles to utility positions
    for i = 1:I
